    ...
    */
    ...
    (function(){
      ...
      const BILL_PENDING_KEY = "karaoke_bill_pending_v1";
      ...
      function createBillIfNotExists(room) {
  if (!room || room.status !== 'using') return;

  let bills = [];
  try {
    bills = JSON.parse(localStorage.getItem(BILL_PENDING_KEY)) || [];
  } catch {
    bills = [];
  }

  // tránh tạo trùng bill cho cùng phòng
  if (bills.some(b => b.roomId === room.id)) return;
  ...
  localStorage.setItem(BILL_PENDING_KEY, JSON.stringify(bills));
      ...
      row.innerHTML = `
          <div>
              ...
              <div style="color:#6b7280;font-size:13px">${money(svc.price)} x ${o.qty}</div>
          </div>
          <div style="display:flex; gap:8px; align-items:center">
              <div style="font-weight:700">${money((svc.price || 0) * o.qty)}</div>
              <button class="btn-small btn-order-decr" data-idx="${idx}"><i class="fa-solid fa-minus"></i></button>
              <button class="btn-small btn-order-incr" data-idx="${idx}"><i class="fa-solid fa-plus"></i></button>
              <button class="btn-small btn-order-del" data-idx="${idx}"><i class="fa-solid fa-trash-can"></i></button>
          </div>`;
